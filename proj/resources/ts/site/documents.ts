namespace Documents {
	...
	function DuplicateInit($elem: UIElement, event: string = 'input'): void {
		$elem.insertAfter(
			el('<span/>', {class: 'glob_print'})
		);
	...
	function EmptyIfZero(th: any): void {
		let $th = el(th.currentTarget);
		if ($th.val() === '0') $th.setVal('');
	}
	...
	class ActController {
		...
		protected $control						: UIElement;
		protected $btns							: UIElement;
		protected $info							: UIElement;
		protected $state						: UIElement;
		protected $btn_new						: UIElement;
		protected $select						: UIElement;
		...
		constructor(selector: string, tables_names: {[key: string]: string}, iters_names: {[key: string]: string}, texts: {[key: string]: string}, csrf: string) {
			...
			/* Set elements */
			this.$view 							= el(selector);
			this.$control 						= el('<div/>', {class: 'control glob_print_tabu'});
			this.$btns 							= el('<div/>', {class: 'btns'});
			this.$btn_new 						= el('<input/>', {type: 'button', class: 'img new', value: this.texts['new']});
			this.$select 						= el('<select/>');
			this.$info 							= el('<div/>', {class: 'info'});
			this.$state 						= el('<span/>', {class: 'state'});
			...
			this.$view.append(
				this.$control.append(
					this.$btns.append(
						this.$btn_new,
						this.$select.append(
							el('<option/>', {value: '', selected: true, disabled: true, hidden: true}).text(this.texts['select'])
						)
					),
					this.$info.append(
						this.$state
		...
		protected AddOption(id: number, datecr: string, name: string): void {
			this.$select.append(
				el('<option/>', {value: id}).text(`«${name}» от ${datecr}`)
			);
		}

		protected OnCreateDocument(): void {
			let $input = el('<input/>', {type: 'text', placeholder: 'Название'});
			Common.Window.Interactive($input, null, [['yes', 'Создать', false]], () => this.CreateDocument($input.closest('.instance')?.find('.space').item(0) as UIElement, String($input.val()).trim()));
		}

		protected CreateDocument($space: UIElement, name: string): void {
			if (name === '') return;
		...
		private DeleteDocument(id: number) {
			...
			this.$select.find(`option[value="${id}"]`).item(0)?.remove();
	...
	class ActDocument {
		...
		private readonly $btns					: UIElement;
		private readonly $btn_add				: UIElement;
		private readonly $btn_print				: UIElement;
		private readonly $wrap					: UIElement;
		private readonly $header				: UIElement;
		private readonly $caption				: UIElement;
		private readonly $contacts				: UIElement;
		private readonly $contact_name			: UIElement;
		private readonly $contact_address		: UIElement;
		private readonly $contact_email			: UIElement;
		private readonly $contact_phone			: UIElement;
		private readonly $contact_date			: UIElement;
		private readonly $lists					: UIElement;

		constructor(id: number, name: string, $container: UIElement, controller: ActController) {
			/* Set variables */
			this.id								= id;
			...
			this.$btns 							= el('<div/>', {class: 'btns glob_print_tabu'});
			this.$btn_add 						= el('<input/>', {type: 'button', value: 'Добавить таблицу', class: 'img add_table'});
			this.$btn_print 					= el('<input/>', {type: 'button', value: 'Печать', class: 'img print'});
			this.$wrap							= el('<div/>', {class: 'wrap'});
			this.$header						= el('<div/>', {class: 'header'});
			this.$caption						= el('<div/>', {class: 'caption'});
			this.$contacts						= el('<div/>', {class: 'contacts'});
			this.$contact_name					= el('<input/>', {type: 'text', placeholder: 'Компания или Ф.И.О ...🖊'});
			this.$contact_address				= el('<input/>', {type: 'text', placeholder: 'Адрес ...🖊'});
			this.$contact_email					= el('<input/>', {type: 'text', placeholder: 'E-mail ...🖊'});
			this.$contact_phone					= el('<input/>', {type: 'text', placeholder: 'Телефон ...🖊'});
			this.$contact_date					= el('<input/>', {type: 'date', class: 'number'});
			this.$lists							= el('<div/>', {class: 'lists'});
			...
			this.$wrap.append(
				...
				this.$header.append(
					...
					this.$contacts.append(
						el('<div/>').append(this.$contact_name),
						el('<div/>').append(this.$contact_address),
						el('<div/>').append(this.$contact_email),
						el('<div/>').append(this.$contact_phone)
					)
				),
				this.$lists,
				el('<div/>', {class: 'footer'}).append(
					el('<div/>').text('место для печати'),
					el('<div/>').append(
						el('<div/>', {class: 'date'}).append(
							el('<div/>').text('Дата:'),
							el('<div/>').append(
								this.$contact_date
							)
						),
						el('<div/>', {class: 'autograph'}).append(
							el('<div/>').text('Автограф:'),
							el('<div/>').append(
								el('<div/>')
							)
						),
		...
		private Fill(): void {
			this.$contact_name.setVal(this.company as string).trigger('input');
			this.$contact_address.setVal(this.address as string).trigger('input');
			this.$contact_email.setVal(this.mail as string).trigger('input');
			this.$contact_phone.setVal(this.phone as string).trigger('input');
			this.$contact_date.setVal(this.date as string).trigger('input');
		}
	...
	class ActTable {
		...
		/* Elements */
		private readonly $container				: UIElement;
		private readonly $list					: UIElement;
		private readonly $wrap					: UIElement;
		private readonly $header				: UIElement;
		private readonly $visible				: UIElement;
		private readonly $remove				: UIElement;
		private readonly $table					: UIElement;
		private readonly $add_line				: UIElement;
		private readonly $collapse				: UIElement;
		private readonly $tr_sum				: UIElement;
		private readonly $sum					: UIElement;
		private readonly $discount				: UIElement;
		private readonly $total					: UIElement;

		constructor(id: number, did: number, data: TypeActTableData | null, $container: UIElement, controller: ActController, document: ActDocument) {
			/* Set variables */
			this.id								= id;
			this.did							= did;
			...
			this.$wrap							= el('<div/>', {class: 'wrap'});
			this.$header	 					= el('<input/>', {type: 'text', placeholder: '...🖊'});
			this.$collapse						= el('<span/>', {class: 'item collapse', title: "Свернуть строки"});
			this.$visible						= el('<span/>', {class: 'visible', title: "Скрыть таблицу"});
			this.$remove						= el('<span/>', {class: 'delete negative', title: "Удалить таблицу"});
			this.$add_line 						= el('<span/>', {class: 'item add', title: 'Добавить строку'});
			this.$table							= el('<table/>');
			this.$tr_sum 						= el('<tr/>');
			this.$sum 							= el('<span/>');
			this.$discount						= el('<input/>', {type: 'text', placeholder: '...🖊'});
			this.$total 						= el('<span/>');
			...
			/* Building DOM */
			this.$list.append(
				el('<div/>', {class: 'control glob_print_tabu'}).append(
					this.$visible,
					this.$remove
				),
				this.$wrap.append(
					el('<div/>', {class: 'title'}).append(this.$header),
					this.$table.append(
						el('<thead/>').append(
							el('<tr/>').append(
								el('<th/>', {class: 'glob_print_tabu'}).append(el('<span/>').text('+/-')),
								el('<th/>').text('Наименование'),
								el('<th/>').text('Количество'),
								el('<th/>').text('Единица измерения'),
								el('<th/>').text('Цена'),
								el('<th/>').text('Сумма')
							)
						),
						el('<tbody/>').append(
							this.$tr_sum.append(
								el('<td/>', {class: 'number glob_print_tabu'}).append(this.$add_line),
								el('<td/>', {colspan: 3}),
								el('<td/>').text('Всего'),
								el('<td/>', {class: 'number'}).append(this.$sum),
							),
							el('<tr/>').append(
								el('<td/>', {class: 'number glob_print_tabu'}).append(
									this.$collapse
								),
								el('<td/>', {colspan: 3}),
								el('<td/>').text('Скидка, %'),
								el('<td/>', {class: 'number'}).append(this.$discount),
							),
							el('<tr/>', {class: 'total'}).append(
								el('<td/>', {class: 'glob_print_tabu'}),
								el('<td/>', {colspan: 3}),
								el('<td/>').text('Итого'),
								el('<td/>', {class: 'number'}).append(this.$total),
							)
						)
					)
				)
		...
		private Fill(): void {
			this.$header.setVal(this.header as string).trigger('input');
			this.$discount.setVal((this.discount || 0).toString()).trigger('blur');
		}
		...
		private EnterPercent(): void {
			this.$discount.setVal(this.GetDiscount().toString());
		}
		...
		private Visible(): void {
			...
			if (this.visible) {
				this.$visible.removeClass('show').setAttributes({'title': 'Скрыть таблицу'});
				this.$wrap.removeClass('hide');
			} else {
				this.$visible.addClass('show').setAttributes({'title': 'Показать таблицу'});
	...
	class ActRecord {
		...
		/* Elements */
		private readonly $before				: UIElement;
		private readonly $tr					: UIElement;
		private readonly $remove				: UIElement;
		private readonly $name					: UIElement;
		private readonly $count					: UIElement;
		private readonly $unit					: UIElement;
		private readonly $price					: UIElement;
		private readonly $sum					: UIElement;

		constructor(id: number, tid: number, data: TypeActRecordData | null, $before: UIElement, controller: ActController, table: ActTable) {
			/* Set variables */
			this.id								= id;
			this.tid							= tid;
			...
			/* Set elements */
			this.$before						= $before;
			this.$tr							= el('<tr/>', {class: 'line'})
			this.$remove						= el('<span/>', {class: 'item del negative', title: 'Удалить строку'});
			this.$name							= el('<input/>', {type: 'text', placeholder: '...🖊'});
			this.$count							= el('<input/>', {type: 'text', placeholder: '...🖊'});
			this.$unit							= el('<input/>', {type: 'text', placeholder: '...🖊'});
			this.$price							= el('<input/>', {type: 'text', placeholder: '...🖊'});
			this.$sum							= el('<span/>');
			...
			/* Building DOM */
			this.$tr.append(
				el('<td/>', {class: 'number glob_print_tabu'}).append(this.$remove),
				el('<td/>').append(this.$name, el('<span/>', {class: 'glob_print'})),
				el('<td/>', {class: 'number'}).append(this.$count),
				el('<td/>', {class: 'number'}).append(this.$unit),
				el('<td/>', {class: 'number'}).append(this.$price),
				el('<td/>', {class: 'number'}).append(this.$sum)
			);
			...
			this.$before.insertBefore(this.$tr);
			...
			this.AutosaveEnable();
			this.Sum();
			this.$sum.text(this.sum.toString());
		...
		private Fill(): void {
			this.$name.setVal(this.name as string).trigger('input');
			this.$count.setVal((this.count || 0).toString()).trigger('blur');
			this.$unit.setVal(this.unit as string).trigger('input');
			this.$price.setVal((this.price || 0).toString()).trigger('blur');
		}
		...
		private EnterCount(): void {
			this.$count.setVal(this.GetCount().toString());
		}
		...
		private EnterPrice(): void {
			this.$price.setVal(this.GetPrice().toString());
		}
	...
	class PriceListDocument {
		...
		private readonly $caption				: UIElement;
		private readonly $contacts				: UIElement;
		private readonly $contact_name			: UIElement/*<HTMLInputElement>*/;
		private readonly $contact_address		: UIElement;
		private readonly $contact_email			: UIElement;
		private readonly $contact_phone			: UIElement;
		private readonly $contact_date			: UIElement;
		...
		constructor(id: number, name: string, $container: UIElement, controller: PriceListController) {
			/* Set variables */
			this.id								= id;
	...
	class PriceListTable {
		...
		constructor(id: number, did: number, data: TypePriceListTableData | null, $container: UIElement, controller: PriceListController, document: PriceListDocument) {
			/* Set variables */
			this.id								= id;
			this.did							= did;
			...
			this.$add_line 						= el('<span/>', {class: 'item add', title: 'Добавить строку'});
			this.$table							= el('<table/>');
			this.$tr_sum 						= el('<tr/>', {class: 'glob_print_tabu'});
			...
			this.$list.append(
				...
				this.$wrap.append(
					...
					this.$table.append(
						...
						el('<tbody/>').append(
							this.$tr_sum.append(
								el('<td/>', {class: 'number items_margin_bottom_cut'}).append(
									el('<div/>').append(this.$add_line),
									el('<div/>').append(this.$collapse)
								),
								el('<td/>', {colspan: 2})
							),
						)
	...
	class PriceListRecord {
		...
		constructor(id: number, tid: number, data: TypePriceListRecordData | null, $before: UIElement, controller: PriceListController, table: PriceListTable) {
			/* Set variables */
			this.id								= id;
			this.tid							= tid;
		...
		private Fill(): void {
			this.$name.setVal(this.name as string).trigger('input');
			this.$price.setVal(this.price as string).trigger('input');
		}
