namespace Common {
	...
	export class Window {
		...
		/* Elements */
		public static $windows: UIElement | null = null;

		static Create(html: string, title: string | null = null): Instance {
			return Window.New(html, title);
		}

		static Interactive(html: string | UIElement, title: string | null, btns: any = [], funcYes: Function | null = null, funcNo: Function | null = null, funcCancel: Function | null = null): Instance {
			let $container = el('<div/>', {class: 'view general interactive'});
			let $content = el('<div/>', {class: 'contain'});
			let $btns = el('<div/>', {class: 'btns'});
			...
			switch (typeof html) {
				case 'string': $content.html(html); break;
				case 'object': $content.append(html); break;
			}

			let _wind = Window.New($container, title);

			for (let i in btns) {
				let $input = el('<input/>', {type: 'button', value: btns[i][1]});
		...
		private static New(html: string | UIElement, title: string | null): Instance {
			if (!Window.$windows) {
				Window.$windows = el('<div/>', {class: 'view general windows'});
	...
	class Instance {
		...
		constructor(id: number, html: string | UIElement, title: string | null) {
			this.id		= id;
...
(typeof (globalThis as any).Common === 'undefined')
	? (globalThis as any).Common = Common
	: Object.assign((globalThis as any).Common, Common);