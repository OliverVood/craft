namespace Base {
	...
	export class Notice {
		...
		private static new(type: TypeNotice, html: string): Instance {
			if (!Notice.$container) {
				...
				Notice.$container.classList.add('component', 'notice', 'container');
	...
	class Instance {
		...
		constructor(id: number, type: TypeNotice, html: string) {
			...
			/* Elements */
			this.$wrap = document.createElement('div'); this.$wrap.classList.add('component', 'notice', 'instance', type);
			...
			let $manager = document.createElement('div'); $manager.classList.add('manager');
			let $close = document.createElement('div'); $close.classList.add('close', 'icon');

			/* Building DOM */
			(Notice.$container as HTMLDivElement).prepend(this.$wrap);
			this.$wrap.append($type, $notice, $manager)
			$notice.append(html);
			$manager.append($close);

			/* Events */
			$manager.addEventListener('click', () => { this.close(); });
		}
