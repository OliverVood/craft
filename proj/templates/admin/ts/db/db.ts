	...
	export namespace DB {
		...
		export class DBInstance {
			...
			constructor(data: DataDB, $structure: HTMLElement) {
				...
				for (const table of data.tables) this.tables[table.name] = new Table(this, table, $structure);
				for (const table of data.tables) {
					for (const key of table.keys.primaries) this.keys.primary[key.name] = new Primary(this, this.tables[table.name], key.name, key.fields);
					for (const key of table.keys.foreigners) this.keys.foreign[key.name] = new Foreign(this, this.tables[table.name], key.name, key.fields, key.relationship_from, {
