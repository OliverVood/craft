namespace Base {
	...
	export class Request {
		...
		public static xhr(address: string, data: BodyInit, options?: RequestOptions): Promise<Response> {
			let xhr = globalThis.Base.GlobalParams.get('xhr');
			let url = `${xhr}${address}`;
			...
			return new Promise((resolve, reject) => {
				fetch(url, Request.getInit(data, options)).then(async response => {
					...
					if (response.status !== 200 && response.status !== 201) {
						globalThis.Base.Response.error(result);
						reject(result);
					}
					resolve(result);
				}).catch(error => {
...
// Проверяем и дополняем Base
if (typeof (globalThis as any).Base === 'undefined') {
	(globalThis as any).Base = Base;
} else {
	// Дополняем существующий Base
